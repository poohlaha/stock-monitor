...
const Cross: React.FC<IShareCrossProps> = memo((props: IShareCrossProps): ReactElement => {
  const padding = 5
    ...
    return (
      <>
        {!Utils.isBlank(labelProps.leftLabel || '') && (
          <>
            <rect
              ...
              y={(props.y || 0) - (leftRectHeight || 0) / 2}
        ...
        {!Utils.isBlank(labelProps.rightLabel || '') && (
          <>
            <rect
              x={(props.width || 0) - (labelProps.rightTextWidth + AxisDefaultProps.lineWidthOrHeight + padding)}
      ...
      !Utils.isBlank(labelProps.bottomLabel || '') && (
        <>
          <rect
            x={rectBX || 0}
            y={rectBY || 0}
            width={(labelProps.bottomTextWidth || 0) + padding}
  ...
  const render = () => {
    ...
    return (
      <svg width={props.width} height={props.height} className={`${props.prefixClassName || ''}-cross`}>
        {/* 纵向 */}
        <line x1={props.x || 0} y1={0} x2={props.x || 0} y2={props.height || 0} stroke={color} strokeDasharray={strokeDasharray} />

        {/* 横向 */}
        <line x1={0} y1={props.y || 0} x2={props.width || 0} y2={props.y || 0} stroke={color} strokeDasharray={strokeDasharray} />
