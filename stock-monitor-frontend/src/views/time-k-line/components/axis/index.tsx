...
const Axis: React.FC<IShareAxisProps> = memo((props: IShareAxisProps): ReactElement => {
  /**
   * 获取涨跌幅
   */
    ...
    return (
      <>
        ...
        >
          {/* x 轴线(减去1避免覆盖 Y 轴 */}
          {props.needAxisXLine && (
            <line x1={0} y1={(props.height || 0) - 0.5} x2={props.width || 0} y2={(props.height || 0) - 0.5} stroke={lineColor} />
          )}

          {/* y 轴线一侧价格线(不画到底，避免与 X 轴重合) */}
          {props.needAxisYLine && (
            <line
              x1={isYLeft ? 0 : props.width || 0}
              y1={0}
              x2={isYLeft ? 0 : props.width || 0}
              y2={props.height || 0}
              stroke={lineColor}
            />
          )}
          ...
          {props.needAxisYLine && (props.yAmplitudes || []).length > 0 && (
            <line
              x1={isYLeft ? props.width || 0 : 0}
              y1={0}
              x2={isYLeft ? props.width || 0 : 0}
              y2={props.height || 0}
              stroke={lineColor}
            />
          )}
              ...
              return (
                <g key={`y-${i}`}>
                  ...
                  {needLine && props.needYLabelLine && (
                    <line x1={line.x1 || 0} y1={line.y1 || 0} x2={line.x2 || 0} y2={line.y2 || 0} stroke={newLineColor} />
                  )}

                  {/* 第一个不要显示刻度线(涨跌幅) */}
                  {needLine && props.needYLabelLine && (
                    <line
                      x1={props.isYLeft ? (props.width  || 0) - AxisDefaultProps.lineWidthOrHeight : 0}
                      y1={line.y1 || 0}
                      x2={props.isYLeft ? (props.width || 0) : AxisDefaultProps.lineWidthOrHeight}
                      y2={line.y2 || 0}
                      stroke={newLineColor}
                    />
                  )}
            ...
            return (
              <g key={`x-${i}`}>
                {needLine && props.needXLabelLine && (
                  <line x1={line.x1 || 0} y1={line.y1 || 0} x2={line.x2 || 0} y2={line.y2 || 0} stroke={lineColor} />
                )}
                <text
                  x={text.x || 0}
                  y={text.y || 0}
